...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  sendMessage: async (conversationId: string, content: string) => {
    ...
    set((state) => {
      ...
      return {
        ...
        messagesById: { ...state.messagesById, [tempId]: conversationId },
      };
    });
    ...
    try {
      ...
      set((state) => {
        ...
        return {
          messages: { ...state.messages, [conversationId]: msgs },
          messagesById: { ...state.messagesById, [sent.id]: conversationId },
        };
      });
    } catch (err) {
      set((state) => {
  ...
  updateMessageStatus: (messageId: string, status: MessageStatus) => {
    set((state) => {
      const convId = state.messagesById[messageId];
      if (!convId || !state.messages[convId]) return state;
